...
class UpgradeShell extends AppShell {
	...
	public function tests() {
		...
		if (! empty ( $this->params ['plugin'] )) {
			$this->_paths = array (
					CakePlugin::path ( $this->params ['plugin'] ) . 'tests' . DS 
			);
		}
		$patterns = array (
				array (
	...
	public function helpers() {
		...
		if (! empty ( $this->params ['plugin'] )) {
			$this->_paths = array (
					CakePlugin::path ( $this->params ['plugin'] ) . 'views' . DS 
			);
		}
		...
		foreach ( $plugins as $plugin ) {
			...
			$pluginHelpers = array_merge ( $pluginHelpers, App::objects ( 'helper', CakePlugin::path ( $plugin ) . DS . 'views' . DS . 'helpers' . DS, false ) );
		}
	...
	public function i18n() {
		...
		if (! empty ( $this->params ['plugin'] )) {
			$this->_paths = array (
					CakePlugin::path ( $this->params ['plugin'] ) 
			);
		}
	...
	public function getOptionParser() {
		...
		$parser->description ( __d ( 'cake_console', "A tool to help automate upgrading an application or plugin " . "from CakePHP 1.3 to 2.0. Be sure to have a backup of your application before " . "running these commands." ) )->addSubcommand ( 'all', array (
