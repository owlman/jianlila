 ...
 * Memcached has some limitations in the amount of
...
class MemcachedEngine extends CacheEngine {
	...
	protected function _setOptions() {
		...
		if (! isset ( $this->_serializers [$serializer] )) {
			throw new CacheException ( __d ( 'cake_dev', '%s is not a valid serializer engine for Memcached', $serializer ) );
		}
		
		if ($serializer !== 'php' && ! constant ( 'Memcached::HAVE_' . strtoupper ( $serializer ) )) {
			throw new CacheException ( __d ( 'cake_dev', 'Memcached extension is not compiled with %s support', $serializer ) );
		}
	 ...
	 * When using memcached as your cache engine
	 ...
	 *
	 * @param bool $check
	 *        	If true no deletes will occur and instead CakePHP will rely
	 *        	on key TTL values.
